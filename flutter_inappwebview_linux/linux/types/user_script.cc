...
FlValue* UserScript::toFlValue() const {
  ...
  if (groupName.has_value()) {
    fl_value_set_string_take(map, "groupName",
                             fl_value_new_string(groupName->c_str()));
  } else {
  ...
  fl_value_set_string_take(map, "forMainFrameOnly",
                           fl_value_new_bool(forMainFrameOnly));
