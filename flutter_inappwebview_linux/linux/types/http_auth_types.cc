...
FlValue* HttpAuthenticationChallenge::toFlValue() const {
  ...
  if (previousFailureCount.has_value()) {
    fl_value_set_string_take(map, "previousFailureCount",
...
FlValue* URLCredential::toFlValue() const {
  ...
  if (username.has_value()) {
    fl_value_set_string_take(map, "username", fl_value_new_string(username.value().c_str()));
  } else {
    fl_value_set_string_take(map, "username", fl_value_new_null());
  }

  if (password.has_value()) {
    fl_value_set_string_take(map, "password", fl_value_new_string(password.value().c_str()));
  } else {
