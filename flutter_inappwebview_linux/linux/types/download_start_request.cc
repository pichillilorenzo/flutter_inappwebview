...
DownloadStartRequest::DownloadStartRequest(WebKitDownload* download)
    : contentLength(-1) {
  if (download == nullptr) {
    return;
  }
...
FlValue* DownloadStartRequest::toFlValue() const {
  ...
  if (mimeType.has_value()) {
    fl_value_set_string_take(map, "mimeType",
                             fl_value_new_string(mimeType.value().c_str()));
  } else {
  ...
  if (userAgent.has_value()) {
    fl_value_set_string_take(map, "userAgent",
                             fl_value_new_string(userAgent.value().c_str()));
  } else {
