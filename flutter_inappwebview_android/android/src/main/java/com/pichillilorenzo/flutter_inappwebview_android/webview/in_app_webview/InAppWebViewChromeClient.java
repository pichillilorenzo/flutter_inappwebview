...
public class InAppWebViewChromeClient extends WebChromeClient implements PluginRegistry.ActivityResultListener, ActivityResultListener {
    ...
    @Nullable
    private ValueCallback<Uri[]> filePathCallbackForL;
    @Nullable
    ...
    //回调
    final WebViewChannelDelegate.ShowFileChooserCallback showFileChooserCallback = new WebViewChannelDelegate.ShowFileChooserCallback() {
        @Override
        public boolean nonNullSuccess(@NonNull List<String> result) {
            StringBuilder sb=new StringBuilder();
            for (String s : result) {
                sb.append(s).append(",");
            }
            System.out.println("onShowFileChooserAndroid:"+sb);
            if (result == null || result.isEmpty())
                InAppWebViewChromeClient.this.filePathCallbackForL.onReceiveValue(new Uri[]{});
            else {
                ...
                InAppWebViewChromeClient.this.filePathCallbackForL.onReceiveValue(uris);
            }
            return true;
        }
    };
