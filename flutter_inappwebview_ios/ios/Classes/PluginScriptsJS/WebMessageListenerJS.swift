...
public class WebMessageListenerJS {
    ...
    public static func WEB_MESSAGE_LISTENER_JS_SOURCE() -> String {
        ...
        window.\(JavaScriptBridgeJS.get_JAVASCRIPT_BRIDGE_NAME())._normalizeIPv6 = function(ip_string) {
        ...
        window.\(JavaScriptBridgeJS.get_JAVASCRIPT_BRIDGE_NAME())._isOriginAllowed = function(allowedOriginRules, scheme, host, port) {
            for (var rule of allowedOriginRules) {
                ...
                if (rule.host != null && rule.host[0] === "[") {
                    try {
                        IPv6 = window.\(JavaScriptBridgeJS.get_JAVASCRIPT_BRIDGE_NAME())._normalizeIPv6(rule.host.substring(1, rule.host.length - 1));
                    } catch {}
                }
                var hostIPv6 = null;
                try {
                    hostIPv6 = window.\(JavaScriptBridgeJS.get_JAVASCRIPT_BRIDGE_NAME())._normalizeIPv6(host);
                } catch {}
